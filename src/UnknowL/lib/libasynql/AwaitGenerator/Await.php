...
namespace UnknowL\lib\libasynql\AwaitGenerator;
...
class Await extends PromiseState{
	...
	private function wakeup(callable $executor) : ?callable{
		...
		if($current === self::ONCE || $current === self::ALL){
			...
			foreach($this->promiseQueue as $promise){
				// if anything is pending, some specific are pending and some specific are resolved, but we will eventually get rejected/resolved from the pending promises
